...
Function GeneralRebuild($vhd)
{
	...
	else
	{
		...
		#Vytvoreni kontrolni souboru s verzi Windows, na kterou bude PC preinstalovano
		if(!(Test-Path "C:\Desired_VHD.txt"))
		{
			echo "$vhd" > "C:\Desired_VHD.txt"
			if($DelDriveECheckBox.Checked -eq $true){"Yes" | out-file "C:\Desired_VHD.txt" -append}else{"No" | out-file "C:\Desired_VHD.txt" -append}
			if($DelDiffCheckBox.Checked -eq $true ){"Yes" | out-file "C:\Desired_VHD.txt" -append}else{"No" | out-file "C:\Desired_VHD.txt" -append}
			write-host "Creating Desired_VHD..." -foreground green
		}
		Write-host "Chosen Windows version"$vhd"..." -foreground green
